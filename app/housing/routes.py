...
def get_housing_price_prediction(
    ...
    housing_median_age: float = Query(
        ge=0, description="Median age of the housing"
    ),
    ...
    ocean_proximity: OceanProximityEnum = Query(
    ...
    df = pd.DataFrame(
        [
            ...
            | {
                "ocean_proximity_" + op.value: 1 if op == ocean_proximity else 0
                for op in OceanProximityEnum
            }
        ]
    )
